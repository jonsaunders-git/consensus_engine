...
from consensus_engine.models import ProposalChoice
...
class CreateProposalChoiceViewTest(TwoUserMixin, TestCase,
    ...
    def test_create_proposal_choice(self):
        dt = timezone.now()
        self.assertTrue(ProposalChoice.objects.filter(text='test choice').count() == 0)
        p = self.create_new_proposal()
        self.assertTrue(p.proposalchoice_set.count() == 0)
        # create choice
        _ = self.getValidView(data={'text': 'test choice',
                                    'priority': '100'}, viewkwargs={'proposal_id': p.id})
        self.assertTrue(ProposalChoice.objects.filter(text='test choice').count() == 1)
        ...
        _ = self.getValidView(data={'text': 'test choice 2',
