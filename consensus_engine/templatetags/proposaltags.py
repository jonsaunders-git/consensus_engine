...
def total_votes(proposal_id):
    total_votes = ChoiceTicket.objects.filter(proposal_choice__deactivated_date__isnull=True, proposal_choice__proposal__id=proposal_id, current=1).count()
...
def my_vote(proposal_id, user_id):
    try:
        my_vote = ProposalChoice.objects.get(deactivated_date__isnull=True, proposal__id=proposal_id, choiceticket__user_id=user_id, choiceticket__current=True).text
    except:
        my_vote = "None"
